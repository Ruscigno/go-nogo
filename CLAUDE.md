...
- **Public URL**: `aviationcortex.com/go-nogo/*` (path-based; this gear's slug is `go-nogo`, abbreviated `gng` in dense contexts per landing-pages brief §4.7.4). Routed via Cloudflared ingress to a local web process on port matching `aviation-cortex/docs/implementation_plan.md` §3.1. Path = slug per the path-consistency decision (landing-pages brief §4.3, 2026-06-02). Abbreviation `gng` is for dense contexts (breadcrumb, gear switcher, analytics labels), NOT URLs.
