...
- **Single-vendor Supabase: Postgres + Auth + RLS.** Browser uses `@supabase/supabase-js` for both auth and simple CRUD; **RLS is the primary authorization layer** for browser-direct calls. The Go backend connects via `pgxpool`, verifies GoTrue JWTs, and scopes every query by `owner_user_id` (research §2.2 / §3).
...
- **Stripe Checkout + Customer Portal + Billing**, two prices: `price_monthly` ($6/mo), `price_annual` ($39/yr) ([ADR-0006](docs/adr/0006-billing-model.md)). Webhook idempotency is a UNIQUE constraint on the Stripe event ID, not application logic (research §2.5).
